/*
 * ============================================================
 *  CENTRAL SERVER - CAMPUS INTER-NETWORKING
 *  Handles TCP departmental messaging and UDP heartbeats.
 * ============================================================
 */
...
Credential valid_credentials[] = {
    {"Islamabad", "NU-ISB-123"}, {"Lahore", "NU-LHR-123"},
    {"Karachi", "NU-KHI-123"},   {"Peshawar", "NU-PSH-123"},
    {"CFD", "NU-CFD-123"},       {"Multan", "NU-MLT-123"}};
  ...
  for (int i = 0; i < MAX_CLIENTS; i++)
    if (clients[i].active && clients[i].udp_address.sin_port != 0)
      sendto(udp_socket_fd, message, strlen(message), 0,
             (struct sockaddr *)&clients[i].udp_address,
             sizeof(clients[i].udp_address));
...
void *handle_client(void *arg) {
  ...
  if (strncmp(buffer, "AUTH:", 5) == 0) {
    ...
    if (campus && password && authenticate_client(campus, password)) {
      ...
      sprintf(log_msg, "[AUTH] %s campus authenticated", campus_name);
      log_message(log_msg);
    } else {
  ...
  if (client_index >= 0) {
    ...
    char log_msg[BUFFER_SIZE];
    sprintf(log_msg, "[DISCONNECT] %s campus disconnected", campus_name);
    log_message(log_msg);
  }
  pthread_mutex_unlock(&clients_mutex);
  close(client_socket);
    ...
    if (recv_len > 0) {
      ...
      if (strncmp(buffer, "HEARTBEAT:", 10) == 0) {
        ...
        if (idx >= 0) {
          ...
          memcpy(&clients[idx].udp_address, &client_addr,
...
int main() {
  ...
  while (1) {
    ...
    int client_socket =
        accept(tcp_socket_fd, (struct sockaddr *)&client_addr, &addr_len);
    if (client_socket < 0) {
