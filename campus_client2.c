/*
 * ============================================================
 *  TERMINAL CLIENT - CAMPUS INTER-NETWORKING
 *  CLI-based client for messaging across campus departments.
 * ============================================================
 */
...
void send_message() {
  ...
  fgets(target_campus, sizeof(target_campus), stdin);
  target_campus[strcspn(target_campus, "\n")] = 0;
  ...
  sprintf(full_msg, "MSG:%s:%s:%s:%s:%s", campus_name, source_dept,
          target_campus, target_dept, message);
...
void view_received_messages() {
  ...
  for (int i = 0; i < inbox_count; i++) {
    char time_str[26];
    struct tm *tm_info = localtime(&inbox[i].timestamp);
    strftime(time_str, 26, "%Y-%m-%d %H:%M:%S", tm_info);
    printf("\nMessage #%d\nTime: %s\nFrom: %s (%s)\nTo: %s (%s)\n%s\n", i + 1,
           time_str, inbox[i].from_campus, inbox[i].from_dept,
           inbox[i].to_campus, inbox[i].to_dept, inbox[i].content);
  }
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 3) {
    strcpy(campus_name, argv[1]);
    strcpy(campus_password, argv[2]);
  } else {
    printf("Available campuses: Islamabad, Lahore, Karachi, Peshawar, CFD, "
           "Multan\n");
    printf("Enter Campus Name: ");
    fgets(campus_name, sizeof(campus_name), stdin);
    campus_name[strcspn(campus_name, "\n")] = 0;
    printf("Enter Campus Password: ");
    fgets(campus_password, sizeof(campus_password), stdin);
    campus_password[strcspn(campus_password, "\n")] = 0;
  }
  ...
  sprintf(auth_msg, "AUTH:%s:%s", campus_name, campus_password);
